...
dfbip.plot.scatter(y='pval',x='log_or',s=0.1,title='BIP')
dfmdd['log_or'] = np.log10(dfmdd['or'])
dfmdd.plot.scatter(y='pval',x='log_or',s=0.1,title='MDD')
...
dfschz_extreme.plot.scatter(y='pval',x='log_or',s=1, title='Schz')
dfadd_extreme=dfadd[(abs(dfadd['log_or'])>1.0) & (dfadd['pval']<0.05)]
dfadd_extreme.plot.scatter(y='pval',x='log_or',s=1,title='ADD')
dfaut_extreme=dfaut[(abs(dfaut['log_or'])>1.0) & (dfaut['pval']<0.05)]
dfaut_extreme.plot.scatter(y='pval',x='log_or',s=1,title='Autism')
dfbip_extreme=dfbip[(abs(dfbip['log_or'])>1.0) & (dfbip['pval']<0.05)]
dfbip_extreme.plot.scatter(y='pval',x='log_or',s=1,title='BIP')
dfmdd_extreme=dfmdd[(abs(dfmdd['log_or'])>1.0) & (dfmdd['pval']<0.05)]
dfmdd_extreme.plot.scatter(y='pval',x='log_or',s=1,title='MDD')
...
dfsnps_oI = dfsnps_oI.sort_values(['log_or','pval'],ascending=[False, True])
dfsnps_oI.head()
...
dfsnps_oI = dfsnps_oI.sort_values(['log_or','pval'],ascending=[True, True])
dfsnps_oI.head()
